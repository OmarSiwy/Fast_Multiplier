...
def visualize_before_after_rich(
    before_heap,
    after_heap,
    stage_num,
    target_height,
...
def _visualize_before_after_rich(
    before_heap,
    after_heap,
    stage_num,
    target_height,
    ...
    console.print(
        "[blue]●[/blue] = FA inputs (3 bits)   [violet]●[/violet] = HA inputs (2 bits)\n"
    )
    ...
    for h in range(max_h - 1, -1, -1):
        ...
        before_row = " ".join(
            (
                "[blue]●[/blue]"
                if (col_idx, h) in fa_input_map
                else (
                    "[violet]●[/violet]"
                    if (col_idx, h) in ha_input_map
                    else "●" if h < len(before_heap.heap[col_idx]) else " "
                )
            )
    ...
    # Counts row
    before_counts = " ".join(
        str(len(before_heap.heap[col_idx])) for col_idx in reversed(range(width))
    )
    after_counts = " ".join(
        str(len(after_heap.heap[col_idx])) for col_idx in reversed(range(width))
    )
    console.print(
        f"{'cnt':>3}   {before_counts:<{col_width}}    {after_counts:<{col_width}}"
    )
