...
class GraphvizGenerator:
    ...
    def _generate_stage0(self, node_id, lines):
        ...
        lines.extend(
            ["  // Stage 0: Initial Partial Products", "  {", "    rank=same;"]
        )
        ...
        for col in range(self.prod_width - 1, -1, -1):
            for bit_idx, (bit_name, bit_type) in enumerate(self.stages[0].heap[col]):
                ...
                lines.append(
                    f'    {node_name} [label="{bit_name}", fillcolor="lightgray"];'
                )
        ...
        if len(all_nodes_in_order) > 1:
            ...
            for i in range(len(all_nodes_in_order) - 1):
                lines.append(
                    f"  {all_nodes_in_order[i]} -> {all_nodes_in_order[i+1]} [style=invis, weight=10];"
                )
            lines.append("")
    ...
    def _generate_reduction_stage(self, stage_idx, node_id, stage_nodes, lines):
        ...
        lines.extend(
            [f"  // Stage {stage_idx + 1}: Reduction", f"  {{", f"    rank=same;"]
        )
        ...
        stage_fas = [
            (col, idx) for s, col, idx, inputs in self.fa_instances if s == stage_idx
        ]
        stage_has = [
            (col, idx) for s, col, idx, inputs in self.ha_instances if s == stage_idx
        ]
                ...
                for i in range(3):
                    if bits_consumed < len(prev_bits):
                        ...
                        if input_type in ["fa_carry", "ha_carry"]:
                            lines.append(
                                f'  {input_node} -> {fa_node} [label="c", color="red"];'
                            )
                        else:
            ...
            for ha_node in ha_nodes_created[col]:
                for i in range(2):
                    if bits_consumed < len(prev_bits):
                        ...
                        if input_type in ["fa_carry", "ha_carry"]:
                            lines.append(
                                f'  {input_node} -> {ha_node} [label="c", color="red"];'
                            )
                        else:
    ...
    def _generate_final_stage(self, node_id, stage_nodes, lines):
        ...
        lines.extend(["  // Final Sum/Carry Outputs", "  {", "    rank=same;"])
        ...
        for col in range(self.prod_width - 1, -1, -1):
            ...
            for bit_idx, (node_name, node_type) in enumerate(final_bits):
                ...
                lines.append(
                    f'    {output_node} [label="{label}", fillcolor="{color}", shape="ellipse"];'
                )
        ...
        if len(output_nodes_in_order) > 1:
            ...
            for i in range(len(output_nodes_in_order) - 1):
                lines.append(
                    f"  {output_nodes_in_order[i]} -> {output_nodes_in_order[i+1]} [style=invis, weight=10];"
                )
            lines.append("")
        ...
        if self.show_final_adder:
            lines.extend(["  // Final Adder", "  {", "    rank=same;"])
            ...
            if len(adder_nodes_in_order) > 1:
                ...
                for i in range(len(adder_nodes_in_order) - 1):
                    lines.append(
                        f"  {adder_nodes_in_order[i]} -> {adder_nodes_in_order[i+1]} [style=invis, weight=10];"
                    )
                lines.append("")

            lines.extend(["  // Product Outputs", "  {", "    rank=same;"])
            ...
            for col, adder_node in sorted(adder_nodes, key=lambda x: -x[0]):
                ...
                lines.append(
                    f'    {prod_node} [label="P[{col}]", fillcolor="lightcyan", shape="ellipse"];'
                )
            ...
            if len(prod_nodes_in_order) > 1:
                ...
                for i in range(len(prod_nodes_in_order) - 1):
                    lines.append(
                        f"  {prod_nodes_in_order[i]} -> {prod_nodes_in_order[i+1]} [style=invis, weight=10];"
                    )
                lines.append("")
