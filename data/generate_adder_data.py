...
def generate_test_data(num_tests=8, width=32, exhaustive=False, output_dir="."):
    """
    ...
    if exhaustive:
        ...
        if width > 8:
            raise ValueError(
                "Exhaustive mode only allowed for width ≤ 8 (too large otherwise)."
            )
            ...
            for b in range(max_val):
                for c_in in (0, 1):
                    if c_in == 1:
                        ...
                        # In hardware: a + ~b + 1
                        b_inverted = ((1 << width) - 1) ^ b  # Bitwise NOT of b
                        result = a + b_inverted + 1
                    else:
                        # Addition mode: a + b
                        result = a + b
    ...
    for i in range(min(3, num_tests)):
        print(
            f"  Test {i}: {a_vals[i]:x} + {b_vals[i]:x} + {c_in_vals[i]} = {c_out_vals[i]}{s_vals[i]:x}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-w", "--width", type=int, default=32, help="Bit width (default: 32)"
    )
    parser.add_argument(
        "-m", "--chunk", type=int, default=4, help="Chunk width (default: 4)"
    )
    parser.add_argument(
        "--exhaustive",
        action="store_true",
