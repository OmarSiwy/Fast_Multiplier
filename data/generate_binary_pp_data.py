...
def binary_pp_compute(y, binary_bit, width):
    ...
    if y & (1 << (width - 1)):  # If negative (MSB set)
        ...
        y_ext = y & (
            (1 << width) - 1
        )  # Mask to width bits, then no sign extension needed
...
def generate_test_data(num_tests=20, width=16, output_dir="data/"):
    """
    ...
    for i in range(min(10, total_tests)):
        ...
        y_signed = (
            y_vals[i] if y_vals[i] < (1 << (width - 1)) else y_vals[i] - (1 << width)
        )
        pp_signed = (
            pp_vals[i] if pp_vals[i] < (1 << width) else pp_vals[i] - (1 << (width + 1))
        )

        print(
            f"  Test {i:2d}: y=0x{y_vals[i]:04x} ({y_signed:6d}), "
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate binary partial product test data"
    )
    parser.add_argument(
        "-n",
        "--num-tests",
        type=int,
        default=20,
        help="Number of random test cases (default: 20)",
    )
    parser.add_argument(
        "-w",
        "--width",
